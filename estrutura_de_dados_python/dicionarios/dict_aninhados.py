contatos = {
    "guilherme@gmail.com": {"nome": "Guilherme", "telefone":"3333-2221"},
    "giovanna@gmail.com": {"nome": "Giovanna", "telefone":"3443-2121"},
    "chappie@gmail.com": {"nome": "Chaipe", "telefone":"3344-9871"},
    "melaine@gmail.com": {"nome": "Melaine", "telefone":"3333-7766", "extra":{"a":"1"}},
}

print(contatos["giovanna@gmail.com"]) #{'nome': 'Giovanna', 'telefone': '3443-2121'}
print(contatos["giovanna@gmail.com"]["telefone"]) #3443-2121

extra = contatos["melaine@gmail.com"]["extra"]["a"]
print(extra)

#em questão de acesso, o python não te restringe. o python te dá muitas possibilidades. A questão da legibilidade é outro tema... rs.